    ...
    <el-row>
      <el-col :span="24">
        <el-radio-group v-model="CategoryId" @change="changeCategoryId">
          <el-radio-button label="7">Windows服务器</el-radio-button>
          <el-radio-button label="8">Linux服务器</el-radio-button>
        </el-radio-group>
        <el-input
          class="search-input r-btn"
          placeholder="输入搜索内容"
          ...
          <el-table-column label="采集状态" align="center" width="120">
            <template slot-scope="scope">
              <el-tag
                ...
                close-transition
                v-text="scope.row.Status === 5 ? '未启用' : '已启用'"
              ></el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="center" width="120">
            <template slot-scope="scope">
              <el-button
                size="mini"
                type="primary"
                @click="details(scope.row.hostId)"
                >详情</el-button
              >
            </template>
          </el-table-column>
  ...
  methods: {
    details(hostId) {
      if (this.CategoryId == 7) {
        this.$router.push("/itgl/win-xq/" + hostId);
      } else {
        this.$router.push("/itgl/linux-xq/" + hostId);
      }
    },
    getlist() {
